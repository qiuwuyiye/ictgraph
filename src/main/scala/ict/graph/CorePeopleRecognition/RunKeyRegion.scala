...
object RunKeyRegion {
	def main(args:Array[String]){
	  //创建环境变量
		val conf = new SparkConf().setAppName("RunKeyRegion")
		val sc = new SparkContext(conf)
		
	  val parser = new scopt.OptionParser[ParaConfig]("RunKeyRegion") {
      head("RunKeyRegion", "1.0")
    ...
    // parser.parse returns Option[Config]
		...
		loader.CriticalRegion(K).map(v=>v._1.toString+"\t"+v._2.toString).saveAsTextFile(outpath)
