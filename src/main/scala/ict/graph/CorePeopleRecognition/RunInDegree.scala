...
object RunInDegree {
	def main(args:Array[String]){
	  //创建环境变量
		val conf = new SparkConf().setAppName("RunInDegree")
		val sc = new SparkContext(conf)
	  //读取参数
		
	  val parser = new scopt.OptionParser[ParaConfig]("RunInDegree") {
      head("RunInDegree", "1.0")
    ...
    val para=parser.parse(args, ParaConfig()).get
		...
		loader.MaxInDegreeNode(K).map(v=>v._1.toString+"\t"+v._2.toString).saveAsTextFile(outpath)
	}
}