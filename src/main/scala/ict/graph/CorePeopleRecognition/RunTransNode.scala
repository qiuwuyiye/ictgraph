...
object RunTransNode {
	def main(args:Array[String]){
	  //创建环境变量
		val conf = new SparkConf().setAppName("RunTransNode")
		val sc = new SparkContext(conf)
		
	  val parser = new scopt.OptionParser[ParaConfig]("RunTransNode") {
      head("RunTransNode", "1.0")
    ...
    // parser.parse returns Option[Config]
		...
		loader.TransRegionalNode(K).map(v=>v._1.toString+"\t"+v._2.toString).saveAsTextFile(outpath)
